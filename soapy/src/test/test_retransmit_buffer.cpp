...
int main() {
    ...
    if(1) {
        rc::check("RetransmitBuffer allocates infinity memory", []() {
            RetransmitBuffer dut;
            std::vector<std::vector<uint32_t>> for_retransmit;

            const int packets  = *rc::gen::inRange(4, 12);

            for(int i = 0; i < packets; i++) {

                std::vector<uint32_t> packet;

                const int psize = *rc::gen::inRange(1, 367);
                for(int j = 0; j < psize; j++) {
                    const int word = *rc::gen::inRange(0u, 0xffffffffu);
                    packet.push_back(word);
                }

                for_retransmit.push_back(packet);

                // we take i as seq number
            }

            dut.sentOverAir(0, for_retransmit);

            const int need_retransmit  = *rc::gen::inRange(0, 11);

            std::vector<uint32_t> idxs = {1};
            int request_retrans_seq = 1;
            dut.scheduleRetransmit(request_retrans_seq, idxs);

            RC_ASSERT( dut.packetsReady() != 0 );

            while( dut.packetsReady() ) {
                auto got = dut.getPacket();
            }
            // for( x : got ) {
