    ...
    return rc::check("Test Short Packet Demodulation", [&]() {
        ...
        for(uint32_t i = 0; i < count; i++) {
            ...
            for(unsigned j = 3; j < sub; j++) {
                transmitted_data.erase(transmitted_data.end()-1);
            }
...
bool test_split(void) {
    ...
    return rc::check("Test FB Split", [&]() {
        ...
        // pass pre
        auto splitFeedbackVectorMapmovScheduledSized = [&] (const std::vector<uint32_t> _d, std::pair<uint32_t,uint32_t> j0, std::pair<uint32_t,uint32_t> j1, const uint32_t lstart, const bool print = true) {
            ...
            for(uint32_t lnow = lstart; built < (int)d.size(); /**/) {
                ...
                if( print ) {
                    cout << lnow <<"," << (lnow%128) << ":    size " << word_size << " bump " << bump << " using size " << using_size << "  (ovr 20: " << ((double)using_size / 20.0) << ")\n";
                }
                ...
                //////////////////////////// work
                {
                    ...
                    set_mapmov_lifetime_32(row, lnow);



                    ret.emplace_back(row);  // vector of vector version
                    // ret.insert(ret.end(), row.begin(), row.end());


                    cout << "loading " << row.size() << "\n";
                }
        ...
        auto joinAfterSplit = [&] (const std::vector<std::vector<uint32_t>> _d) {
        ...
        // const uint32_t data_size = (43 + 43  + 85 )          *20;
        // const uint32_t data_size = (43 + 1 )          *20;
        const uint32_t data_size =   ( *rc::gen::inRange(0u, 4000u) )        *20;

        std::vector<uint32_t> _d;
        generate_input_data_base(_d, data_size, 0x77000000, false);
        airTx->padData(_d);

        const std::vector<uint32_t> d = _d;
        ...
        const uint32_t lifetime_start =   43 +  (( *rc::gen::inRange(0u, 4000u) ) *128) ;
        ...
        const std::pair<uint32_t,uint32_t> jsingle = {85*slicewc,128};
        // const std::pair<uint32_t,uint32_t> j1 = {42*slicewc,(43+42)};


        // auto results = splitFeedbackVectorMapmovScheduledSized(d, j0, j1, lifetime_start);
        auto results = splitFeedbackVectorMapmovScheduledSized(d, jsingle, jsingle, lifetime_start);

        auto resultsJoined = joinAfterSplit(results);

        RC_ASSERT(resultsJoined == d);
