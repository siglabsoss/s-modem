...
const exp_data_1280_ext_15_cplx = mutil.IShortToComplexMulti(exp_data_1280_ext_15);


// resulting vector is vector a times conj(vector b)
function conj_mult(a,b) {

  let d = [];
  let len = Math.min(a.length,b.length);

  for(let i = 0; i < len; i++) {
    let r = a[i].mul(mt.conj(b[i]));
    d.push(r);
  }
...
// do not use
// this was an older function and does not give ability to set offset into exp_data_1280_ext_15
function coarse_sync(a,b) {

  // console.log('a len ' + a.length);
  // console.log('b len ' + b.length);

  let stlen = Math.min(a.length, b.length);

  // let asreal_a = a.map((e,i)=>e.re);

  let ym = conj_mult(a,b);

  // let asreal_ym = ym.map((e,i)=>e.re);
  // let asarg_ym = ym.map((e,i)=>e.arg());

  let expcomplex = mutil.IShortToComplexMulti(exp_data_1280_ext_15); // exp data

  // console.log('expcomplex len ' + expcomplex.length );
  // console.log('ym len ' + ym.length );

  let am_inner = mult(ym,expcomplex.slice(0,stlen));
...
// This stream represents the functionality of cs00's Coarse Sync
// see https://beta.observablehq.com/@drom/ofdm-symbol-synchronisation/2 for the formula used
// input: IShort
// output: IShort
class CoarseSync extends Stream.Transform {
  constructor(options={}, options2={}, callback=null) {
    super(options); //Passing options to native constructor (REQUIRED)

    const {print1=false, print2=false, print3=false, print4=false, print5=false, ofdmNum=20, triggerAtStart=false, cp=256} = options2;

    if( (ofdmNum % 4) !== 0 || (ofdmNum % 5) !== 0 ) {
      throw(new Error('ofdmNum must be evenly divisible by 4 and 5'));
    }

    Object.assign(this, {print1,print2,print3,print4,print5,ofdmNum,triggerAtStart,cp});

    if( callback === null || typeof callback !== 'function') {
        throw(new Error('CoarseSync 3rd argument must be function'));
    }

    this._sync_callback = callback;

    this.lifetime_frame = 0; // lifetime frames through this class

    this.pending_trigger = false;
    this.doing_coarse = false;

    this.pending_trigger_at = null;

    this.sz = 1024; // affects chunk size and exp_data_1280_ext_15_cplx size
    this.byte_sz = this.sz*4;
    this.byte_sz_cp = this.cp*4;
    this.byte_sz_with_cp = (this.sz + this.cp)*4;

    this._resetCoarseSyncMembers();

    if( this.triggerAtStart ) {
      this.pending_trigger = true;
    }

    let exp_source = exp_data_1280_ext_15_cplx; // change this to use a different exponential for coares sync
    this.exp = []; // indexed in the order in which they are used, not by address

    // pre slice exp wave and store
    for(let i = 0; i < 5; i++) {
      // let bufferIndex = this.getExpIndexArg(i+1);
      let bufferOffset = this.getExpOffsetArg(i+1);
      // console.log(`${i} ${bufferOffset.toString(16)} `);
      this.exp[i] = exp_source.slice(bufferOffset, bufferOffset+this.sz);
    }

    // the _flatten nonsense is becuase lodash does not have repeat :(
    this.zeros = Buffer.from(_.flatten(_.times(this.byte_sz, _.constant([0]))));

  }

  triggerCoarse() {
    if( this.pending_trigger ) {
        console.log("Warning: pending_trigger was already set. dropping the coarse sync request");
    }
    this.pending_trigger = true;
  }

  triggerCoarseAt(x) {
    this.pending_trigger_at = x;
  }

  _resetCoarseSyncMembers() {
    this.chunk0 = undefined;
    this.capture = [];
    this.triggered_frame = 0; // how many frames into this trigger are we looking at
    this.cumulative_sum = mt.complex(0,0);
  }

  
  _transform(chunk1_raw, encoding, cb) {
    if(chunk1_raw.length !== this.byte_sz_with_cp) {
      throw(new Error(`CoarseSync stream must have ${this.byte_sz_with_cp/4} uint32 chunks`));
    }

    if( !this.doing_coarse && this.pending_trigger_at !== null ) {
      if( this.pending_trigger_at === this.lifetime_frame ) {
        this.pending_trigger = true;
        this.pending_trigger_at = null;
      }
    }

    if( !this.doing_coarse && this.pending_trigger ) {
      this.doing_coarse = true;
      this.pending_trigger = false;
      this._resetCoarseSyncMembers();
      if( this.print4 ) {
        console.log("Trigger began at frame " + this.lifetime_frame);
      }
    }

    if( this.doing_coarse ) {

      if(this.triggered_frame === 0) {

        // let longData = Buffer.concat([this.capture, chunk]);
        // let len = longData.length;

        let keep = chunk1_raw.slice(0, this.byte_sz);
        this.capture = chunk1_raw.slice(this.byte_sz);

        this.chunk0 = this.convBufferComplex(keep);
        // this.chunk0_u32 = Array.from(this.convBuffer(chunk1_raw));
        this.triggered_frame++;
        this.lifetime_frame++;
        this.push(this.zeros);
        cb();
        return;
      }

      if(this.print5) {
        console.log("entering at " + this.triggered_frame);
      }

      this.capture = Buffer.concat([this.capture, chunk1_raw]);

      while(this.capture.length >= this.byte_sz ) {

        if(this.print5) {
          console.log(this.capture.length);
        }

        let use      = this.capture.slice(0, this.byte_sz);
        this.capture = this.capture.slice(this.byte_sz); // keep
        ...
        let chunk1 = this.convBufferComplex(use);
        // let chunk1_u32 = Array.from(this.convBuffer(chunk1_raw));


        let ym = conj_mult(this.chunk0,chunk1);

        // console.log(`ym is ${ym.length} and ${ym[0].toString()}`);

        let expBuffer = this.getExpBuffer();

        if(this.print2) {
          // mutil.printHexNl(this.chunk0_u32.slice(0,8), 'input_conj_multi_location_0 ' + this.triggered_frame);
          // mutil.printHexNl(chunk1_u32.slice(0,8), 'input_conj_multi_location_1 ' + this.triggered_frame);
          mutil.printHexNl(mutil.complexToIShortMulti(ym.slice(0,8)), 'output_conj_multi_location ' + this.triggered_frame);
          mutil.printHexNl(mutil.complexToIShortMulti(expBuffer.slice(0,8)), 'exp_data ' + this.triggered_frame);
        }
        ...
        this.cumulative_sum = this.cumulative_sum.add(sum);
        ...
        // console.log(`${this.triggered_frame} had angle: ${this.cumulative_sum.arg()}`);
        // console.log(`${this.triggered_frame} runs offset: ${this.getExpOffset().toString(16)}`);

        if( this.triggered_frame == (this.ofdmNum-1) ) {
          if( this.print3 ) {
            console.log(`finishing up, angle is ${this.cumulative_sum.arg()}`);
          }

          let riscv_angle_mag = mutil.riscvATANComplex(this.cumulative_sum);

          let temp_angle = riscv_angle_mag&0xffff;
          let temp_offset = ((((temp_angle+6553)&0xffff)*1280)>>16);

          this._sync_callback(temp_offset, this.cumulative_sum);
          this.doing_coarse = false;
        ...
        this.chunk0 = chunk1;
        // this.chunk0_u32 = chunk1_u32;
        this.triggered_frame++;
        this.lifetime_frame++;


        // we are consuming 1280 on the input.  during coarse sync. we still consume this many......

        // push zeros
        // this was the original idea for cs00 but we scrapped it due to code complexity
        this.push(this.zeros);
      }
      // console.log(this.triggered_frame-1);
      ...
      this.push(chunk1_raw.slice(0, this.byte_sz)); // push first 1024, not tail, this is same as cs00
      cb();
    }
  } // transform 
  ...
  getExpIndexArg(arg) {
    const key = Math.max(arg-1,0);
    const offset = (5-(key%5));
    return offset;
  }

  // returns offset in to exp_data_1280_ext_15
  getExpOffsetArg(arg) {
    const offset = this.getExpIndexArg(arg);
    const ret = offset * 0x100;
    return ret;
  ...
  getExpOffset() {
    return this.getExpOffsetArg(this.triggered_frame);
  }

  // returns cached exp_data_1280_ext_15 that's already been converted and sliced
  getExpBuffer() {
    let index = (this.triggered_frame-1) % 5;
    // console.log(`${this.triggered_frame} maps to ${index}`);
    return this.exp[index];
  }

  // takes the input type of Buffer, and converts it to a typed array of uint32
  convBuffer(chunk) {
    // does array copy
  ...
  // takes the input type of Buffer, and converts it to Complex()
  convBufferComplex(chunk) {
    return mutil.IShortToComplexMulti(this.convBuffer(chunk));
  }
}
